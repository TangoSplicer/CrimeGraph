...
| Security and workflow suite | `npm run test` | Covers encrypted media, storage-secret format, high-risk confirmation, secure wipe, hostile import bounds, forensic dossier v1–v4 integrity and ledger/context tamper detection, controlled markings, explainable quality findings and saved-query reasons, supervisory review, operator lifecycle, field task access closure, assignment visibility, case-playbook lifecycle, source-bound local-lead promotion, offline QR label binding, physical exhibit custody, stated observation uncertainty, and deterministic briefing generation. |
...
| Evidence derivative ledger | Operator-authored records remain parent-evidence-bound, hash-digested, audited, and non-destructive; signed dossiers include them and verify legacy v1 dossiers. |
| Exhibit custody and QR label | Offline QR payload matches its local evidence-provenance binding; physical movements require reauthentication, append custody events, and deny field alteration. |
...
| Security review | The audit ledger verifies after lifecycle, assignment, task handoff, field evidence, derivative/annotation creation, saved-query lifecycle, case-playbook/lead exercises, exhibit custody events, observation-context updates, reproducible-briefing creation, controlled dossier, and supervisory review exercises. |
