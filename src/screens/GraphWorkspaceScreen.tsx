...
export const GraphWorkspaceScreen: React.FC = () => {
  ...
  return (
    <div className="flex flex-col w-full h-screen pb-safe-nav bg-[#0c0e14] relative">
      ...
      {isAnalysisOpen && (
        <div className="absolute top-[60px] right-4 z-30 max-h-[calc(100dvh-9rem-env(safe-area-inset-bottom))] overflow-y-auto bg-[#1c2030] border border-[#2ecc71]/60 rounded-lg shadow-xl w-72 p-3 pb-[calc(1rem+env(safe-area-inset-bottom))] mt-safe">
      ...
      <BottomSheet isOpen={(!!selectedNodeId || !!selectedEdgeId) && !connectingFromId} onClose={() => { setSelectedNodeId(null); setSelectedEdgeId(null); setIsEditingNode(false); }} title={selectedNode ? (isEditingNode ? 'Edit Node' : selectedNode.data.label) : 'Relationship Details'}>
        ...
        {selectedNode && isEditingNode && (
          <div className="space-y-4">
            ...
            <div className="sticky bottom-0 -mx-4 flex space-x-3 border-t border-[#252a3a] bg-[#14171f] px-4 pt-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
