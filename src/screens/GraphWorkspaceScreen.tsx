...
export const GraphWorkspaceScreen: React.FC = () => {
  ...
  const handleCreateBriefing = async () => {
    if (!activeCaseId || isCreatingBriefing) return;
    if (briefingTitle.trim().length < 3) { setBriefingMessage('Enter a briefing title of at least three characters.'); return; }
    if (briefingPurpose.trim().length < 5) { setBriefingMessage('State a briefing purpose of at least five characters.'); return; }
    if (!briefingNodeIds.length && !briefingNoteIds.length) { setBriefingMessage('Select at least one local intelligence record or note before building the briefing.'); return; }
    try {
      ...
      setBriefingTitle(''); setBriefingPurpose(''); setBriefingNodeIds([]); setBriefingNoteIds([]);
    } catch (error) {
      setBriefingMessage(error instanceof Error ? error.message : 'Briefing could not be created.');
    } finally {
  ...
  return (
    <div className="flex flex-col w-full h-screen pb-safe-nav bg-[#0c0e14] relative">
      ...
      {isAnalysisOpen && (
        <div className="absolute top-[60px] right-4 z-30 max-h-[calc(100dvh-9rem-env(safe-area-inset-bottom))] overflow-y-auto bg-[#1c2030] border border-[#2ecc71]/60 rounded-lg shadow-xl w-72 p-3 pb-safe-action mt-safe">
          <div className="flex justify-between items-center gap-2 border-b border-[#252a3a] pb-2 mb-3">
            <h3 className="text-[10px] font-bold text-[#2ecc71] uppercase tracking-widest">Case Structure Analysis</h3>
            <div className="flex items-center gap-2"><span className="text-[9px] text-[#7880a0]">No person-level scoring</span><button type="button" onClick={() => setIsAnalysisOpen(false)} aria-label="Close analysis" className="rounded border border-[#454d66] px-2 py-1 text-[9px] font-bold uppercase text-[#dde1ec]">Close</button></div>
          </div>
      ...
      {isPlaybookOpen && (
        <div className="fixed inset-0 z-[90] flex items-end sm:items-center justify-center bg-black/80 p-safe-modal">
          <section role="dialog" aria-modal="true" aria-label="Case playbook" className="safe-modal-card w-full max-w-md rounded-lg border border-[#3a7bd5] bg-[#14171f] p-5 shadow-2xl">
      ...
      {isLeadRegisterOpen && (
        <div className="fixed inset-0 z-[90] flex items-end sm:items-center justify-center bg-black/80 p-safe-modal">
          <section role="dialog" aria-modal="true" aria-label="Local lead register" className="safe-modal-card w-full max-w-md rounded-lg border border-[#f39c12] bg-[#14171f] p-5 shadow-2xl">
      ...
      <BottomSheet isOpen={(!!selectedNodeId || !!selectedEdgeId) && !connectingFromId} onClose={() => { setSelectedNodeId(null); setSelectedEdgeId(null); setIsEditingNode(false); }} title={selectedNode ? (isEditingNode ? 'Edit Node' : selectedNode.data.label) : 'Relationship Details'}>
        ...
        {selectedNode && isEditingNode && (
          <div className="space-y-4">
            ...
            <div className="sticky bottom-0 -mx-4 flex space-x-3 border-t border-[#252a3a] bg-[#14171f] px-4 pt-3 pb-safe-action">
              <button type="button" onClick={() => setIsEditingNode(false)} className="min-h-11 flex-1 py-3 border border-[#454d66] text-[#dde1ec] rounded text-xs font-bold uppercase">Cancel</button>
              <button type="button" onClick={handleSaveEdit} disabled={!editLabel.trim()} className="min-h-11 flex-1 py-3 bg-[#3a7bd5] text-white rounded text-xs font-bold uppercase disabled:bg-[#252a3a]">Save Changes</button>
            </div>
          </div>
        )}
