...
**Scope:** Device-bound encrypted storage, device posture, supervisory review, secure operator lifecycle, assigned field work queues, and Capacitor 8 Android release assurance.
...
| Security and workflow suite | `npm run test` | Covers encrypted media, storage-secret format, high-risk confirmation, secure wipe, hostile import bounds, supervisory review, operator lifecycle, assignment visibility, and field access closure. |
...
Complete and archive the full [physical-device acceptance protocol](./DEVICE_ACCEPTANCE.md) before authorizing distribution. Its mandatory checks include encrypted first-run commissioning, restart persistence, secure operator lifecycle actions, assigned field queues, unassigned-access blocking, camera evidence capture, review/resubmission, session locks, verified pairing, secure wipe, and Android backup/transfer exclusion.
...
| Evidence and review | Encrypted media, provenance digest, pending review, return, correction, and resubmission are validated offline. |
...
| Security review | The audit ledger verifies after lifecycle, assignment, field evidence, and supervisory review exercises. |
